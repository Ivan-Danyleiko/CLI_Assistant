...
class AddressBook(UserList):
    ...
    def __str__(self):
        ...
        for account in self.data:
            if account['birthday']:
                birdth = account['birthday'].strftime("%d/%m/%Y")
            else:
                birdth = ''
            if account['phones']:
                new_value = []
    ...
    def add(self, record):
        try:
            if record and record.name:
                ...
                self.log("Failed to add contact. Invalid record or missing name.")
    ...
    @staticmethod
    def create_record(name, phones, birdth, email, status, note):
        phone_list = [phone.strip() for phone in phones.split(',')]
        data = {
            'name': name,
            'phones': phone_list,
            'birthday': Birthday(birdth).value,
            'email': email,
            'status': status,
            'note': note
        }
